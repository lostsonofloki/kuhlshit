...
- **Upcoming shows — Add to calendar:** **`GigTracker.jsx`** / **`GigTracker.css`** resolve per-show Google / `.ics` payloads via **`resolveCalendarForUpcomingShow`** in **`src/lib/upcomingShowCalendar.js`**; **`ArtistDetailPage.jsx`** passes calendar-related props into **GigTracker**. **`AddToCalendarRow.jsx`** / **`AddToCalendarRow.css`** support **`customDetails`**, **`includeSeriesHub`**, **`labelText`**, and a compact variant for tight layouts. Hub UIs (**`ClosedOnSundaysPage.jsx`**, **`ClosedOnSundays.css`**, **`UpcomingAtAlsStrip.jsx`**, **`UpcomingAtAlsStrip.css`**) surface add-to-calendar on upcoming rows; **`closedOnSundayHubEvents.js`** adds **`getCosHubEventCalendar`** so hub rows reuse the same calendar details.
...
- **Branding — series title:** user-facing copy normalized to **Closed on Sundays** (plural) across **`data.json`**, **`AddToCalendarRow.jsx`**, **`closedOnSundayHubEvents.js`**, **`ClosedOnSundayLivePromo.jsx`**, **`ClosedOnSundaysPage.jsx`**, **`closedOnSundayLivePromoSchedule.js`**, **`HomePage.css`** (comments), and related COS strings.
