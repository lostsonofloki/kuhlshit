...
async function scrapeHaysop() {
  console.log('🎸 Scraping Haysop links...\n');
  ...
  try {
    ...
    console.log('📄 Navigating to hypeddit link...');
    await page.goto('https://hypeddit.com/haysop/haysop?utm_source=ig&utm_medium=social&utm_content=link_in_bio&fbclid=PAZXh0bgNhZW0CMTEAc3J0YwZhcHBfaWQMMjU2MjgxMDQwNTU4AAGnCaP_Vbgo46vxcut6J_-GtSQM8qS6A_TWZvKpXvjZefrua0DT6tl6TvalN0M_aem_WXEaZWXO2O-vbzfjJAdfCA', {
    ...
    const scrapedData = await page.evaluate(() => {
      ...
      links.forEach(link => {
        ...
        if (href && !href.includes('hypeddit.com') && text) {
          data.links.push({
            text: text,
            url: href
          });
    ...
    let artistIndex = currentData.artists.findIndex(a => 
      a.name.toLowerCase().includes('haysop')
    );
    ...
    if (artistIndex !== -1) {
      ...
      if (socialLinks.website && !socialLinks.website.includes('hypeddit')) {
      ...
      console.log('❌ Haysop not found in data.json');
    }
...
scrapeHaysop().catch(console.error);
