...
function buildCalendarUrl(event) {
  ...
  // Google all-day dates are end-exclusive, so add one day.
...
function PorchFestPage() {
  ...
  useEffect(() => {
    const onScroll = () => setShowMapFab(window.scrollY > 300);
    onScroll();
  ...
  const handleMapFabClick = () => {
    if (!firstMapUrl) return;
    window.open(firstMapUrl, "_blank", "noopener,noreferrer");
  };

  const getDayAnchorId = (dayName) =>
    `lineup-${String(dayName || "day")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")}`;
  ...
  return (
    <>
                    ...
                    <div className="event-info">
                      ...
                      {event.pricing && (
                        <div className="pricing-row">
                          <span className="price-badge-lg">
                            {event.pricing.dayPass}/Day
                          </span>
                          <span className="price-badge-lg highlight">
                      ...
                      {event.lineup?.length > 0 ? (
                        <>
                          <div className="pf-day-jump" aria-label="Jump to day">
                            {event.lineup.map((daySchedule) => (
                              <a
                                key={`jump-${daySchedule.day}`}
                                href={`#${getDayAnchorId(daySchedule.day)}`}
                                className={`pf-day-jump-link${
                                  daySchedule.day === todayWeekday
                                    ? " is-today"
                                    : ""
                                }`}
                              >
                                {daySchedule.day} ({daySchedule.artists?.length || 0})
                              </a>
                            ))}
                          </div>

                          <div className="pf-lineup-by-day">
                            {event.lineup.map((daySchedule) => (
                              <section
                                key={`section-${daySchedule.day}`}
                                id={getDayAnchorId(daySchedule.day)}
                                className="pf-day-section"
                              >
                                <div className="pf-day-section-header">
                                  <h4>{daySchedule.day}</h4>
                                  <span>
                                    {daySchedule.artists?.length || 0} artists
                                  </span>
                                </div>
                      ...
                      {event.location.socialLinks && (
                        <div className="venue-socials">
                          {event.location.socialLinks.facebook && (
                            <a
                              href={event.location.socialLinks.facebook}
                              target="_blank"
                              rel="noopener noreferrer"
                          ...
                          {event.location.socialLinks.instagram && (
                            <a
                              href={event.location.socialLinks.instagram}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="social-link"
      ...
      {showMapFab && firstMapUrl && (
        <button
          type="button"
          className="map-fab"
          aria-label="Open festival map in Google Maps"
